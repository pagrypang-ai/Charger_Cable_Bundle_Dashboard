...
def canonical_column_name(column) -> str:
    ...
    aliases = {
        ...
        "model number / product id": "Model Number / Product ID",
        ...
        "connect type/ports": "Connect Type/Ports",
...
def infer_product_type(row) -> str:
    existing = normalize_product_type(row.get("Product Type", ""))
    if existing != "Unknown":
        return existing

    text = " ".join(
        str(row.get(column, "") or "")
        for column in ["Product Name", "Bundle", "Pack", "Connect Type/Ports", "Charging Tech"]
    ).lower()
    has_length = pd.notna(parse_cable_length(row.get("Cable Length", "")))
    has_bundle_text = "bundle" in text or ("charger" in text and "cable" in text) or ("adapter" in text and "cable" in text)
    has_cable_text = any(term in text for term in [" cable", "cable ", "cord", "usb-c to", "usb-a to", "lightning to", "micro-usb"])
    has_charger_text = any(term in text for term in ["charger", "adapter", "wall charger", "power adapter"])

    if has_bundle_text:
        return "Charger + Cable Bundle"
    if has_length or has_cable_text:
        return "Cable"
    if has_charger_text or pd.notna(parse_number(row.get("Max Output Power", ""))):
        return "Charger"
    return "Unknown"


def product_type_options(df: pd.DataFrame) -> list[str]:
    source = df["Product Type Group"] if "Product Type Group" in df.columns else df.get("Product Type", pd.Series(dtype=str))
    seen = {normalize_product_type(value) for value in source.dropna()}
...
def prepare_data(raw: pd.DataFrame):
    ...
    df["Product Type Group"] = df.apply(infer_product_type, axis=1)
